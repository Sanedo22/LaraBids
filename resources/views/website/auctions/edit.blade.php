                    ...
                    <form action="{{ route('auctions.update', $auction->id) }}" method="POST" enctype="multipart/form-data" id="auctionEditForm" novalidate>
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <small class="text-muted">Minimum amount each next bid must increase by (Max ₹1000).</small>
                                @error('min_increment')
                                    <div class="invalid-feedback d-block" data-server-error>{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group input-group-lg">
                                    ...
                                    <input type="text" name="start_time" id="start_time_picker" class="form-control bg-light border-0 shadow-none @error('start_time') is-invalid @enderror" 
                                        value="{{ old('start_time', $auction->start_time->format('Y-m-d H:i')) }}" {{ $hasBids ? 'disabled' : '' }} placeholder="Select Date & Time">
                                </div>
                                @if($hasBids)
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group input-group-lg">
                                    ...
                                    <input type="text" name="end_time" id="end_time_picker" class="form-control bg-light border-0 shadow-none @error('end_time') is-invalid @enderror" 
                                        value="{{ old('end_time', $auction->end_time->format('Y-m-d H:i')) }}" placeholder="Select Date & Time">
                                </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (startPickerElement && endPickerElement) {
            const startPicker = flatpickr(startPickerElement, {
                ...
                // If it's already in the past (locked or ongoing), we allow keeping it but not picking new past dates
                minDate: startPickerElement.value && new Date(startPickerElement.value) < new Date() ? startPickerElement.value : new Date(),
